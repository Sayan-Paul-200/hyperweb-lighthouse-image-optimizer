...
final class PluginTest extends TestCase {
	...
	public function test_create_builds_composition_root(): void {
		...
		self::assertCount( 24, $plugin->providers() );
		...
		self::assertInstanceOf( CriticalImageMetaBox::class, $plugin->providers()[11] );
		self::assertInstanceOf( CriticalImageAssets::class, $plugin->providers()[12] );
		self::assertInstanceOf( ElementorHeroBackgroundMetaBox::class, $plugin->providers()[13] );
		self::assertInstanceOf( WooCommerceIntegration::class, $plugin->providers()[14] );
		self::assertInstanceOf( ElementorIntegration::class, $plugin->providers()[15] );
		self::assertInstanceOf( ReconciliationWorker::class, $plugin->providers()[16] );
		self::assertInstanceOf( OptimizationWorker::class, $plugin->providers()[17] );
		self::assertInstanceOf( LoadingAttributeManager::class, $plugin->providers()[18] );
		self::assertInstanceOf( ResponsivePreloadManager::class, $plugin->providers()[19] );
		self::assertInstanceOf( ElementorCriticalBackgroundPreloadManager::class, $plugin->providers()[20] );
		self::assertInstanceOf( ElementorBackgroundStylesheetManager::class, $plugin->providers()[21] );
		self::assertInstanceOf( DeliveryManager::class, $plugin->providers()[22] );
		self::assertInstanceOf( I18n::class, $plugin->providers()[23] );
	}
