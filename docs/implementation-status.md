...
- Frontend delivery now includes a conservative same-site uploads URL resolver for unresolved `wp_content_img_tag` fragments, and page inventory now labels resolved or unresolved local uploads URL evidence for Diagnostics follow-up.
- Frontend delivery now also includes a narrow Elementor widget-content bridge for confirmed static attachment-backed Image, Image Box, CTA, and site-logo widgets. The bridge reuses the shared delivery transformer, preserves widget wrappers and non-image attributes, allows repeated isolated Elementor widget originals to transform independently, and keeps gallery, carousel, slideshow, editor/preview, multiple-image, script-triggered fallback, and unresolved contexts fail-open.
- Diagnostics issue reporting now includes readiness evidence for structured Elementor background findings, distinguishing ready derivatives, missing derivatives, disabled enabled-format settings, and excluded attachments while leaving slideshow/custom-CSS/script-triggered originals as reporting-only cases.
...
Phase 7 now includes derivative URL resolution, responsive modern source-set building, safe picture rendering, active `wp_get_attachment_image` plus `wp_content_img_tag` delivery integrations, an internal emergency rollback switch, cache-invalidation request hooks, missing-derivative diagnostics, and conservative local uploads URL fallback resolution for unresolved content-image fragments. The phase remains unchecked until supported WordPress frontend delivery and cache-integration smoke tests are performed.
...
- `ContentIssueReportService` adds conservative, fixed-code findings for missing modern derivatives, oversized full-size selection, missing responsive candidates, missing intrinsic dimensions, critical image lazy loading, heuristic below-the-fold eager loading, external images, animated GIFs, unresolved local uploads URLs, broken local image references, supported structured Elementor CSS-background usage, and possible duplicate source downloads.
...
- Dashboard statistics reconciliation now explicitly scans normal `inherit` Media Library attachments, preventing a fresh cache from showing all-zero attachment counts while bulk scan/queue views see optimized or queued images.
...
- [x] Dashboard statistics scans include normal `inherit` attachments so cached dashboard counters align with Media Library and bulk-scan state.
