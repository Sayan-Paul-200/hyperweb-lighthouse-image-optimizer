<?php
/**
 * Conversion result sanitization.
 *
...
/**
 * Redacts sensitive or server-local data from conversion result messages/details.
 */
final class ConversionResultSanitizer {
	...
	public function sanitize_message( string $message ): string {
		return $this->truncate_string(
			$this->redact_paths_in_string( trim( $message ) ),
	 ...
	 * @param bool         $truncated Whether anything was truncated.
	 * @return array<mixed>
	 */
	...
	private function is_sensitive_key( string $key ): bool {
		return 1 === preg_match(
			'/(password|passwd|token|secret|cookie|nonce|authorization|api[_-]?key|bearer|auth|stack[_-]?trace|trace|exception)/i',
			$key
		);
	}

	/**
	 * Redact likely absolute paths from a string.
	 *
